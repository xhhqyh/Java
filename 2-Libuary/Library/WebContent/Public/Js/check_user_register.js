...
function check(){
	...
	if(checkUserName() && checkPassword() && checkRepassword() && checkUserTrueName() && checkPost() && checkUserAddress() && checkUserTel() && checkUserEmail()){
		return true;
	}else{
		return false;
	}
...
/*验证前台注册用户的表单*/
 ...
 function checkUserTrueName(){
   ...
   //验证邮编
 function checkPost(){
 	var name=document.getElementById("user-post").value.trim();
 	var nameRegex=/^[1-9][0-9]{5}$/;
 	if(!nameRegex.test(name)){
 		document.getElementById("birhdayInfo").innerHTML="<span style='color:red;'>*您输入的格式有误,请重新填写邮编</span>";
 		return false;
 	}else{
 ...
 function checkUserAddress(){
 	...
 	if(!nameRegex.test(name)){
 		document.getElementById("AddressInfo").innerHTML="<span style='color:red;'>*您输入的家庭住址不规范,5个汉字以上</span>";
 		return false;
 	}else{
