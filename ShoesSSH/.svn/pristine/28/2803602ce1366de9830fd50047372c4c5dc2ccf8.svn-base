...
<body>
	<table id="evaluteDg" class="easyui-datagrid" title="商品信息"
		style="width: 1570px; height: 770px"
		data-options="singleSelect:false,
						  collapsible:true,
						  toolbar: '#evaluteToolbar',
						  url:'evalute!getEvaluteInfo',
						  method:'get',
						  fit:true,
						  fitColumns:true,
						  pagination:true,
		...
		<thead>
			<tr>
				...
				<th data-options="field:'evaluteInformation',width:210,align:'center'">咨询问题</th>
				<th data-options="field:'evaluteTime',width:200,align:'center'">咨询时间</th>
				<th data-options="field:'evaluteContent',width:200,align:'center',formatter:formatContent">回复内容</th>
				<th data-options="field:'manager',width:220,align:'center',formatter:formatManger">管理员</th>
		...
		<form id="evaluteForm" action="evalute!saveEvalute" method="get">
			<table cellpadding="5">
	    		...
	    		<tr>
	    			<td>咨询时间:</td>
	    			<td>
	    				<input id="evaluteTime" readOnly="readonly" name="evalute.evaluteTime" class="easyui-textbox" data-options="required:true,width:220"/>
	    			</td>
	    		</tr>
	    		<tr>
	    			<td>客服回复:</td>
	    			<td>
	    				...
	    				<!-- 评论星级 -->
	    				<input id="evaluteLevel" type="hidden" name="evalute.evaluteLevel"/>
	    			</td>
	    		</tr>		    		    				
			</table>
		</form>	
	...
	<script type="text/javascript">	
		...
		function formatContent(value, row, index) {
			if (value != null && value != "") {
				...
				return '<font color="red">无回复</font>';
			}
		}
		//获得品牌名字
		...
		//判断鞋子款式
		function formatManger(value, row, index) {
			if (value != null) {
				return value.managerName;
			} else{
				return '无';
			}
		}
		...
		function formatDo(value, row, index) {
			if (value != null && value == 0 ) {
				return '<a href="#" id="back" onclick="openWin(2)">回复</a>';
			} else{
		...
		function openWin(flag) {			
			if (flag == 2) {
				var chkRows = $("#evaluteDg").datagrid("getChecked");				  
					...
					$("#evaluteLevel").val(chkRows[0].evaluteLevel);										
